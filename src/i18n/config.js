...
if (window.localStorage.getItem('stemLang') === null) {
  if (langSub === 'zh') {
    localStorage.setItem('stemLang', 'CN')
    lan = 'CN'
  } else {
    localStorage.setItem('stemLang', 'EN')
    lan = 'EN'
  }
  i18n.locale = lan
} else { // 根据用户上次的选择的语言来显示语言
  i18n.locale = window.localStorage.getItem('stemLang')
}
